    ...
    ui(new Ui::CompraFactura)
{
    ...
    */
    ...
    ui->lineEdit_descuento->installEventFilter(this);
    ui->lineEdit_embalaje->installEventFilter(this);
...
QString CompraFactura::getID()
{
    return id;
}
QString CompraFactura::getPersonaID()
{
    return persona_id;
}
QString CompraFactura::getFechaDeclaracion()
{
    return ui->dateEdit_declaracion->date().toString("dd-MM-yyyy");
}
QString CompraFactura::getFechaEmision()
{
    return ui->dateTimeEdit_emision->date().toString("dd-MM-yyyy");
}
QString CompraFactura::getSerie()
{
    return ui->lineEdit_serie->text();
}
QString CompraFactura::getNumero()
{
    return ui->lineEdit_numero->text();
}
QString CompraFactura::getCodigo()
{
    return ui->lineEdit_cod->text();
}
QString CompraFactura::getNombre()
{
    return ui->lineEdit_nombre->text();
}
QVector<QVector<QString> > CompraFactura::getOrdenes(){
...
QVector<QVector<QString> > CompraFactura::getGuias(){
                           ...
                           , QString codigo, QString nombre)
{
    ...
    if(query.exec(str_query)){
        ...
        qDebug()<<"fecha_emision: "<<fecha_emision<<endl;
        QDateTime dt;
        ...
        ui->lineEdit_descuento->setText(query.value(1).toString());
        ui->lineEdit_embalaje->setText(query.value(2).toString());
                                 ...
                                 , QString descripcion, QString p_total)
{
    ...
    embalaje = ui->lineEdit_embalaje->text().toDouble();
    descuento = ui->lineEdit_descuento->text().toDouble();
...
bool CompraFactura::guardar()
{
    ...
    if (id.compare("") == 0) {
        ...
        str_query += ", "+ui->lineEdit_descuento->text();
        str_query += ", "+ui->lineEdit_embalaje->text();
        str_query += ", '"+ui->dateEdit_declaracion->date().toString("yyyy-MM-dd")+"'";
        ...
        */
        ...
        str_query += ", descuento = "+ui->lineEdit_descuento->text();
        str_query += ", embalaje = "+ui->lineEdit_embalaje->text();
...
void CompraFactura::on_lineEdit_descuento_textChanged(const QString& arg)
{
    disconnect(ui->tableWidget, SIGNAL(itemChanged(QTableWidgetItem*))
               , this, SLOT(on_tableWidget_itemChanged(QTableWidgetItem*)));
...
void CompraFactura::on_lineEdit_embalaje_textChanged(const QString& arg)
{
    disconnect(ui->tableWidget, SIGNAL(itemChanged(QTableWidgetItem*))
               , this, SLOT(on_tableWidget_itemChanged(QTableWidgetItem*)));

    double embalaje, descuento;
    embalaje = arg.toDouble();
    descuento = ui->lineEdit_descuento->text().toDouble();
...
{    
        ...
        if(e->type() == QEvent::KeyPress){
            ...
            case Qt::Key_Return:
                if(QString(typeid(*QApplication::focusWidget()).name()) == "18QExpandingLineEdit"){
                    ...
                    //ui->lineEdit_descuento->setFocus(Qt::TabFocusReason);
                }
                break;
            case Qt::Key_Enter:{
                QKeyEvent* key = new QKeyEvent(QEvent::KeyPress, Qt::Key_Return, Qt::NoModifier);
    ...
    if(obj == w_temp){
        if(e->type() == QEvent::KeyPress){
            ...
            {
            case Qt::Key_Return:{
                ui->lineEdit_embalaje->setFocus(Qt::TabFocusReason);
                return true;
            }break;
            case Qt::Key_Enter:{
                QKeyEvent* key = new QKeyEvent(QEvent::KeyPress, Qt::Key_Return, Qt::NoModifier);
...
void CompraFactura::on_pushButton_guardar_dolar_clicked()
{
    bool result = false;
    QString str_query;
    QSqlQuery query;

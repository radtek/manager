...
void SunatConsultaRUC::on_thread_loadWeb()
{
    view_consultaRuc->load(tr("http://e-consultaruc.sunat.gob.pe/cl-ti-itmrconsruc/jcrS00Alias"));
    //view_consultaRuc->show();
    //view_consultaRuc->lower();
}
void SunatConsultaRUC::on_thread_finished()
{
    delete sender();
}
...
{			
        ...
        //qDebug() << "NO SON IGUALES" << endl;
    {
        ...
        */
        ...
        app_dir = app_dir + "/tessdata/";

        qDebug() << app_dir << endl;

        int r = api->Init(app_dir.toStdString().c_str(), "eng", tesseract::OEM_DEFAULT, NULL, 0, NULL, NULL, true);
        //qDebug() << "r: " << r << endl;
        if (r) {

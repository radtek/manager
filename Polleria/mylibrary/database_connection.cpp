...
database_connection::database_connection()
{
    ...
    file.setFileName(temporal+QString(DB_FILE_NAME_EXTENSION));
    if(!file.exists())
    {
        if(file.open(QIODevice::WriteOnly | QIODevice::Text)){
            //qDebug()<<"File created."<<endl;
...
void database_connection::mf_load_temporal()
{
    QString name_file_cfg = QString() + temporal + DB_FILE_NAME_EXTENSION;
...
{    
    ...
    if(md_host == "" && md_port == "" && md_name == "" && md_user == "" && md_pass == ""){
        return false;
    }
    if(my_database.open()){
        return true;

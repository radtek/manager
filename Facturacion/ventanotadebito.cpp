...
#include "ui_ventanotadebito.h"

VentaNotaDebito::VentaNotaDebito(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::VentaNotaDebito)
{
    ui->setupUi(this);
    ...
    ui->tableWidget->setItemDelegateForColumn(INDEX_AUMENTO_P_TOTAL, dSpinBoxDelegate);
...
{
    qDebug()<<"delete venta nota debito"<<endl;
    delete ui;
}

void VentaNotaDebito::set_widget_previous(QWidget *widget_previous)
{
    this->widget_previous = widget_previous;
}
void VentaNotaDebito::on_ventaSNSelector_closing()
{
                          ...
                          , QString codigo, QString nombre, QString direccion)
{
    ...
    str_query += " WHERE documento_h_documento.documento_id = "+id+")";
    str_query += " UNION ALL";
    str_query += "(SELECT nota_debito.declaracion, nota_debito.monto, nota_debito.anulado, '', '', '', '' FROM nota_debito";
    ...
    if(query.exec(str_query)){
        ...
        while(query.next()){
            ...
            QString aumento_p_total = query.value(6).toString();
...
void VentaNotaDebito::set_cliente(QString persona_id, QString codigo, QString nombre, QString direccion)
{
    /*
    this->persona_id = persona_id;
    this->codigo = codigo;
...
void VentaNotaDebito::set_producto(QString producto_id
                                , QString cantidad, QString unidad
                                , QString descripcion, QString p_total, QString aumento_p_total)
{
...
void VentaNotaDebito::set_data(QString persona_id, QString tipo_persona_id
...
void VentaNotaDebito::set_data(int tipo, QString id, QString persona_id, QString tipo_persona_id
...
void VentaNotaDebito::on_producto_closing()
{
...
void VentaNotaDebito::on_pushButton_cliente_clicked()
{
...
void VentaNotaDebito::on_pushButton_ing_prod_clicked()
{
...
void VentaNotaDebito::on_pushButton_guardar_clicked()
{
    ...
    if(!(ui->lineEdit_serie->text().compare(serie) == 0 && ui->lineEdit_numero->text().compare(numero) == 0)){
        ...
        str_query += " JOIN anexo ON anexo.documento_id = nota_debito.comprobante_documento_id";
        str_query += " ORDER BY nota_debito.comprobante_documento_id DESC, anexo.numero DESC LIMIT 1)";
...
void VentaNotaDebito::on_pushButton_eliminar_clicked()
{
    if(id.compare("") == 0) {
...
void VentaNotaDebito::on_pushButton_imprimir_clicked()
{
    if(id.compare("") == 0){
        qDebug()<<"is id"<<endl;
        return;
...
void VentaNotaDebito::on_pushButton_amarres_clicked()
{

}

void VentaNotaDebito::on_pushButton_salir_clicked()
{
...
void VentaNotaDebito::closeEvent(QCloseEvent *ce)
{
    emit closing();
    ce->accept();
}
bool VentaNotaDebito::eventFilter(QObject *obj, QEvent *e)
{
    QWidget* w_temp;
    w_temp = this;
    if(obj == w_temp){
...
void VentaNotaDebito::on_pushButton_up_clicked()
{
    QTableWidgetItem* item= ui->tableWidget->currentItem();
    if(item){
...
void VentaNotaDebito::on_pushButton_down_clicked()
{
    QTableWidgetItem* item= ui->tableWidget->currentItem();
    if(item){
        int column = item->column();
...
void VentaNotaDebito::on_pushButton_borrar_clicked()
{
    QTableWidgetItem* item= ui->tableWidget->currentItem();
    if(!item)return;
    int row = item->row();
    ...
    for(int i = 0; i < rowCount; i++) {
        QString aum_p_total = ui->tableWidget->item(i, INDEX_AUMENTO_P_TOTAL)->text();

        double p_total = aum_p_total.toDouble();
...
void VentaNotaDebito::on_lineEdit_serie_returnPressed()
{
...
void VentaNotaDebito::on_tableWidget_itemChanged(QTableWidgetItem *item)
{
    disconnect(ui->tableWidget, SIGNAL(itemChanged(QTableWidgetItem*))
                , this, SLOT(on_tableWidget_itemChanged(QTableWidgetItem*)));
...
void VentaNotaDebito::on_boleta_buscar_closing()
{
...
void VentaNotaDebito::on_pushButton_buscar_boleta_clicked()
{
    if(persona_id.compare("") == 0){
...
void VentaNotaDebito::on_pushButton_jalar_boleta_clicked()
{
...
void VentaNotaDebito::on_pushButton_quitar_boleta_clicked()
{
...
void VentaNotaDebito::on_factura_buscar_closing()
{
    VentaBuscar* w = (VentaBuscar*)sender();
...
void VentaNotaDebito::on_pushButton_buscar_factura_clicked()
{
    if(persona_id.compare("") == 0){
        QMessageBox::warning(this, "Advertencia", "No ha ingresado el cliente.", "Ok");
        return;
...
void VentaNotaDebito::on_pushButton_jalar_factura_clicked()
{
...
void VentaNotaDebito::on_pushButton_quitar_factura_clicked()
{
...
void VentaNotaDebito::on_pushButton_anular_clicked()
{
    if(id.compare("") == 0){
    ...
    QString str_query = "UPDATE nota_debito SET";
    str_query += " anulado = '"+QString().setNum(anulado)+"'";
    str_query += " WHERE comprobante_documento_id = "+id;
...
void VentaNotaDebito::on_doubleSpinBox_monto_valueChanged(double arg1)
{
    double total = 0.0;
    int rowCount = ui->tableWidget->rowCount();

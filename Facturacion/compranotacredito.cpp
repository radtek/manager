    ...
    ui(new Ui::CompraNotaCredito)
{
    ...
    ui->tableWidget->setItemDelegateForColumn(INDEX_DESC_CANT, delegate);
    ...
    ui->tableWidget->setItemDelegateForColumn(INDEX_DESC_P_TOTAL, delegate);
...
QString CompraNotaCredito::getID()
{
    return id;
}
QString CompraNotaCredito::getPersonaID()
{
    return persona_id;
}
QString CompraNotaCredito::getFechaDeclaracion()
{
    return ui->dateEdit_declaracion->date().toString("dd-MM-yyyy");
}
QString CompraNotaCredito::getFechaEmision()
{
    return ui->dateTimeEdit_emision->date().toString("dd-MM-yyyy");
}
QString CompraNotaCredito::getSerie()
{
    return ui->lineEdit_serie->text();
}
QString CompraNotaCredito::getNumero()
{
    return ui->lineEdit_numero->text();
}
QString CompraNotaCredito::getCodigo()
{
    return ui->lineEdit_codigo->text();
}
QString CompraNotaCredito::getNombre()
{
    return ui->lineEdit_nombre->text();
}
QVector<QVector<QString> > CompraNotaCredito::getFacturas()
{
...
QVector<QVector<QString> > CompraNotaCredito::getBoletas()
{
    ...
    if (id.compare("") == 0) {
        ...
        */
        ...
        for(int i= 0; i< ui->tableWidget->rowCount(); i++){
            ...
            str_query_2 += QString()+",("+this->id+", "+producto_id+", "+cantidad+", "+total+", "+desc_cantidad+", "+desc_p_total+")";
        }
...
void CompraNotaCredito::on_pushButton_guardar_dolar_clicked()
{
    bool result = false;
    QString str_query;
    QSqlQuery query;
...
void CompraNotaCredito::on_lineEdit_monto_textChanged(const QString &arg1)
{
    double total = 0.0;
    int rowCount = ui->tableWidget->rowCount();

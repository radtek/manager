...
#include "ui_ventanotacredito.h"

VentaNotaCredito::VentaNotaCredito(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::VentaNotaCredito)
{
    ui->setupUi(this);
    ...
    ui->comboBox_id_boleta->installEventFilter(this);
    ...
    ui->comboBox_id_factura->installEventFilter(this);
...
{
    qDebug()<<"delete venta nota credito"<<endl;
    delete ui;
}

void VentaNotaCredito::set_widget_previous(QWidget *widget_previous)
{
    this->widget_previous = widget_previous;
}
void VentaNotaCredito::on_ventaSNSelector_closing()
{
                          ...
                          , QString codigo, QString nombre, QString direccion)
{
    ...
    str_query += " WHERE documento_h_documento.documento_id = "+id+")";
    str_query += " UNION ALL";    
    str_query += "(SELECT nota_credito.declaracion, nota_credito.monto, nota_credito.anulado, '', '', '', '' FROM nota_credito";
    ...
    if(query.exec(str_query)){        
        ...
        ui->doubleSpinBox_monto->setValue(query.value(1).toDouble());
        anulado = query.value(2).toInt();
        if(!anulado){
...
void VentaNotaCredito::set_cliente(QString persona_id, QString codigo, QString nombre, QString direccion)
{
    /*
    this->persona_id = persona_id;
    this->codigo = codigo;
...
void VentaNotaCredito::set_producto(QString producto_id
...
void VentaNotaCredito::set_data(QString persona_id, QString tipo_persona_id
              ...
              , QVector<QVector<QString>> productos)
{
    ...
    for(int i=0; i<productos.size(); i++){
        QString producto_id = productos[i][0];
...
void VentaNotaCredito::set_data(int tipo, QString id, QString persona_id, QString tipo_persona_id
                           ...
                           , QVector<QVector<QString>> productos)
{
    ...
    case venta_items::BOLETA:{
        ui->comboBox_id_boleta->addItem(id);
        ui->comboBox_serie_numero_boleta->addItem(serie+" - "+numero);
    }break;
    case venta_items::FACTURA:{
        ui->comboBox_id_factura->addItem(id);
        ui->comboBox_serie_numero_factura->addItem(serie+" - "+numero);
    }break;
    }
...
bool VentaNotaCredito::guardar()
{
    ...
    if (id.compare("") == 0) {
        ...
        for(int i= 0; i< ui->tableWidget->rowCount(); i++){
            ...
            QString desc_cant = ui->tableWidget->item(i, INDEX_DESC_CANT)->text();
            QString desc_p_total = ui->tableWidget->item(i, INDEX_DESC_P_TOTAL)->text();

            str_query_2 += QString()+",((SELECT MAX(documento.id) FROM documento), "+producto_id+", "+cantidad+", "+total+", "+desc_cant+", "+desc_p_total+")";
        }
...
void VentaNotaCredito::set_time()
{
    ui->dateTimeEdit_sistema->setDateTime(QDateTime::currentDateTime());
}

void VentaNotaCredito::on_pushButton_transformar_clicked()
{
    ...
    case 0:{
        ...
        if(id.compare("") == 0){
            ...
            w->set_data(venta_items::NOTA_CREDITO, id, persona_id, tipo_persona_id
            , fecha_emision, ""
            , serie, numero
            , codigo, nombre
            , direccion, productos);
...
void VentaNotaCredito::on_cliente_closing()
{
...
void VentaNotaCredito::on_producto_closing()
{
...
void VentaNotaCredito::on_pushButton_cliente_clicked()
{
...
void VentaNotaCredito::on_pushButton_ing_prod_clicked()
{
...
void VentaNotaCredito::on_pushButton_guardar_clicked()
{
    ...
    if(!(ui->lineEdit_serie->text().compare(serie) == 0 && ui->lineEdit_numero->text().compare(numero) == 0)){
        QString str_query = "(SELECT IF(EXISTS(SELECT 1 FROM nota_credito";
        ...
        str_query += " JOIN anexo ON anexo.documento_id = nota_credito.comprobante_documento_id";
        str_query += " ORDER BY nota_credito.comprobante_documento_id DESC, anexo.numero DESC LIMIT 1)";
...
void VentaNotaCredito::on_pushButton_eliminar_clicked()
{
    if(id.compare("") == 0) {
...
void VentaNotaCredito::on_pushButton_imprimir_clicked()
{
    if(id.compare("") == 0){
        qDebug()<<"is id"<<endl;
        return;
    ...
    if(ui->lineEdit_serie->text().length() == 4){
        ...
        str_query += " AND series.tipo_documento_id = "+QString().setNum(tipo_documento::NOTA_CREDITO);
    ...
    label = new QLabel(ui->lineEdit_subtotal->text(), 0);
    label->setObjectName(ui->lineEdit_subtotal->objectName());
    labels.push_back(label);
    label = new QLabel(ui->label_igv_value->text(), 0);
    label->setObjectName(ui->label_igv_value->objectName());
    labels.push_back(label);
    label = new QLabel(ui->lineEdit_igv->text(), 0);
    label->setObjectName(ui->lineEdit_igv->objectName());
    labels.push_back(label);
    ...
    w->set_tipo_documento(tipo_documento::NOTA_CREDITO, ui->lineEdit_serie->text(), id_series
                          , labels, width, height, margen_fila, margen_columna);

    SYSTEM->change_center_w(this, w);
}

void VentaNotaCredito::on_pushButton_amarres_clicked()
{

}

void VentaNotaCredito::on_pushButton_salir_clicked()
{
...
void VentaNotaCredito::showEvent(QShowEvent *se)
{
    if(focusWidget()){
...
void VentaNotaCredito::hideEvent(QHideEvent *he)
{
    emit hiding();
    he->accept();
}
void VentaNotaCredito::closeEvent(QCloseEvent *ce)
{
    emit closing();
    ce->accept();
}
bool VentaNotaCredito::eventFilter(QObject *obj, QEvent *e)
{
    QWidget* w_temp;
    w_temp = this;
    if(obj == w_temp){
    ...
    if(obj == w_temp){
        if(e->type() == QEvent::KeyPress){
            ...
            {
            case Qt::Key_Return:{
                ui->comboBox_serie_numero_boleta->setFocus(Qt::TabFocusReason);
                return true;
            }break;
            }
        }
    ...
    if(obj == w_temp){
        if(e->type() == QEvent::KeyPress){
            ...
            {
            case Qt::Key_Return:{
                ui->comboBox_serie_numero_factura->setFocus(Qt::TabFocusReason);
                return true;
            }break;
            }
        }
...
void VentaNotaCredito::on_pushButton_up_clicked()
{
    QTableWidgetItem* item= ui->tableWidget->currentItem();
    if(item){
...
void VentaNotaCredito::on_pushButton_down_clicked()
{
    QTableWidgetItem* item= ui->tableWidget->currentItem();
    if(item){
        int column = item->column();
...
void VentaNotaCredito::on_pushButton_borrar_clicked()
{
    QTableWidgetItem* item= ui->tableWidget->currentItem();
    if(!item)return;
    int row = item->row();
...
void VentaNotaCredito::on_lineEdit_serie_returnPressed()
{
...
void VentaNotaCredito::on_tableWidget_itemChanged(QTableWidgetItem *item)
{
    disconnect(ui->tableWidget, SIGNAL(itemChanged(QTableWidgetItem*))
               , this, SLOT(on_tableWidget_itemChanged(QTableWidgetItem*)));
...
void VentaNotaCredito::on_boleta_buscar_closing()
{
...
void VentaNotaCredito::on_pushButton_buscar_boleta_clicked()
{
    if(persona_id.compare("") == 0){
...
void VentaNotaCredito::on_pushButton_jalar_boleta_clicked()
{
...
void VentaNotaCredito::on_pushButton_quitar_boleta_clicked()
{
...
void VentaNotaCredito::on_factura_buscar_closing()
{
    VentaBuscar* w = (VentaBuscar*)sender();
...
void VentaNotaCredito::on_pushButton_buscar_factura_clicked()
{
    if(persona_id.compare("") == 0){
        QMessageBox::warning(this, "Advertencia", "No ha ingresado el cliente.", "Ok");
        return;
...
void VentaNotaCredito::on_pushButton_jalar_factura_clicked()
{
...
void VentaNotaCredito::on_pushButton_quitar_factura_clicked()
{
...
void VentaNotaCredito::on_pushButton_anular_clicked()
{
    if(id.compare("") == 0){
    ...
    QString str_query = "UPDATE nota_credito SET";
    str_query += " anulado = '"+QString().setNum(anulado)+"'";
    str_query += " WHERE comprobante_documento_id = "+id;
...
void VentaNotaCredito::on_dateEdit_declaracion_dateChanged(const QDate &date)
{
...
void VentaNotaCredito::on_dateEdit_emision_dateChanged(const QDate &date)
{
...
void VentaNotaCredito::on_doubleSpinBox_monto_valueChanged(double arg1)
{
    double total = 0.0;
    int rowCount = ui->tableWidget->rowCount();

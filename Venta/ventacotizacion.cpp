...
#include "ui_ventacotizacion.h"

VentaCotizacion::VentaCotizacion(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::VentaCotizacion)
{
    ui->setupUi(this);
    ...
    ui->dateEdit_emision->setDateTime(QDateTime::currentDateTime());
...
VentaCotizacion::~VentaCotizacion()
{
    qDebug()<<"delete venta cotizacion"<<endl;
    delete ui;
}

void VentaCotizacion::set_widget_previous(QWidget *widget_previous)
{
    this->widget_previous = widget_previous;
}
void VentaCotizacion::on_ventaSNSelector_closing()
{
...
void VentaCotizacion::next_serie_numero()
{
...
bool VentaCotizacion::select(QString id
...
void VentaCotizacion::set_cliente(QString persona_id, QString codigo, QString nombre, QString direccion)
{
    /*
    this->persona_id = persona_id;
    this->codigo = codigo;
...
void VentaCotizacion::set_producto(QString producto_id
...
void VentaCotizacion::set_data(QString persona_id, QString tipo_persona_id
...
void VentaCotizacion::set_data(int tipo, QString id, QString persona_id, QString tipo_persona_id
...
bool VentaCotizacion::guardar()
{
    ...
    if (id.compare("") == 0) {
        ...
        str_query += QString().setNum(tipo_documento::COTIZACION);
        str_query += ", 1)";
        str_query += "&&END_QUERY&&";
        ...
        // BOLETA
        str_query += "INSERT INTO cotizacion(comprobante_documento_id";
        str_query += ")VALUES(";
        str_query += "(SELECT MAX(documento.id) FROM documento))";
        str_query += "&&END_QUERY&&";
...
void VentaCotizacion::set_time()
{
    ui->dateTimeEdit_sistema->setDateTime(QDateTime::currentDateTime());
}

void VentaCotizacion::on_pushButton_transformar_clicked()
{
    int tipo = ui->comboBox_transformar->currentIndex();

    switch(tipo){
    case 0:{
        tipo = tipo_documento::REGISTRO_SIN_DOCUMENTO;
        ...
        if(id.compare("") == 0){
            ...
            w->set_data(venta_items::COTIZACION, id, persona_id, tipo_persona_id
            , fecha_emision, ""
            , serie, numero
            , codigo, nombre
            , direccion, productos);
    ...
    case 3:{
        tipo = tipo_documento::NOTA_PEDIDO;
    ...
    case 5:{
        tipo = tipo_documento::COTIZACION;
...
void VentaCotizacion::on_dateEdit_emision_dateChanged(const QDate &date)
{

}
void VentaCotizacion::on_cliente_closing()
{
...
void VentaCotizacion::on_producto_closing()
{
...
void VentaCotizacion::on_pushButton_cliente_clicked()
{
...
void VentaCotizacion::on_pushButton_ing_prod_clicked()
{
...
void VentaCotizacion::on_pushButton_guardar_clicked()
{
    ...
    if(!(ui->lineEdit_serie->text().compare(serie) == 0 && ui->lineEdit_numero->text().compare(numero) == 0)){
        QString str_query = "(SELECT IF(EXISTS(SELECT 1 FROM cotizacion";
        ...
        str_query += " JOIN anexo ON anexo.documento_id = cotizacion.comprobante_documento_id";
        str_query += " ORDER BY cotizacion.comprobante_documento_id DESC, anexo.numero DESC LIMIT 1)";
...
void VentaCotizacion::on_pushButton_eliminar_clicked()
{
    if(id.compare("") == 0) {
...
void VentaCotizacion::on_pushButton_imprimir_clicked()
{
    if(id.compare("") == 0){
        return;
    }
    ...
    if(ui->lineEdit_serie->text().length() == 4){        
        ...
        str_query += " AND series.tipo_documento_id = "+QString().setNum(tipo_documento::COTIZACION);
    ...
    w->set_tipo_documento(tipo_documento::COTIZACION, ui->lineEdit_serie->text(), id_series
                          , labels, width, height, margen_fila, margen_columna);

    SYSTEM->change_center_w(this, w);
}

void VentaCotizacion::on_pushButton_amarres_clicked()
{

}

void VentaCotizacion::on_pushButton_salir_clicked()
{
...
void VentaCotizacion::showEvent(QShowEvent *se)
{
    if(focusWidget()){
...
void VentaCotizacion::hideEvent(QHideEvent *he)
{
    emit hiding();
    he->accept();
}
void VentaCotizacion::closeEvent(QCloseEvent *ce)
{
    emit closing();
    ce->accept();
}
bool VentaCotizacion::eventFilter(QObject *obj, QEvent *e)
{
    QWidget* w_temp;
    w_temp = this;
    if(obj == w_temp){
...
void VentaCotizacion::on_pushButton_up_clicked()
{
    QTableWidgetItem* item= ui->tableWidget->currentItem();
    if(item){
...
void VentaCotizacion::on_pushButton_down_clicked()
{
    QTableWidgetItem* item= ui->tableWidget->currentItem();
    if(item){
        int column = item->column();
...
void VentaCotizacion::on_pushButton_borrar_clicked()
{
    QTableWidgetItem* item= ui->tableWidget->currentItem();
    if(!item)return;
    int row = item->row();
...
void VentaCotizacion::on_lineEdit_serie_returnPressed()
{
...
void VentaCotizacion::on_tableWidget_itemChanged(QTableWidgetItem *item)
{
    disconnect(ui->tableWidget, SIGNAL(itemChanged(QTableWidgetItem*))
               , this, SLOT(on_tableWidget_itemChanged(QTableWidgetItem*)));

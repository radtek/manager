...
#include "ui_ventafactura.h"

VentaFactura::VentaFactura(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::VentaFactura)
{
    ui->setupUi(this);
    ...
    igv = SYSTEM->get_igv();    
    // SET CUSTOMS
    ui->label_igv_value->setText(QString().setNum(igv, ' ', 3));
    //ui->widget_tb_custom->set_tableWidget(ui->tableWidget);
...
{
    qDebug()<<"delete venta factura"<<endl;
    delete ui;
}

void VentaFactura::set_widget_previous(QWidget *widget_previous)
{    
    this->widget_previous = widget_previous;
}
void VentaFactura::on_ventaSNSelector_closing()
{
...
void VentaFactura::next_serie_numero()
{
...
bool VentaFactura::select(QString id
                          ...
                          , QString codigo, QString nombre, QString direccion)
{
    ...
    str_query += " WHERE documento_h_documento.documento_id = "+id+")";
    str_query += " UNION ALL";
    str_query += "(SELECT factura.declaracion, factura.anulado, '', '', '' FROM factura";
    str_query += " WHERE factura.comprobante_documento_id = "+id+")";
    str_query += " UNION ALL";
...
void VentaFactura::set_cliente(QString persona_id, QString codigo, QString nombre, QString direccion)
{
    /*
    this->persona_id = persona_id;
    this->codigo = codigo;
...
void VentaFactura::set_producto(QString producto_id
...
void VentaFactura::set_data(QString persona_id, QString tipo_persona_id
              , QDateTime dt_emision, QDateTime dt_sistema, QString serie, QString numero
              , QString codigo, QString nombre, QString direccion
              , QVector<QVector<QString>> productos)
{
    if(tipo_persona_id.toInt() == tipo_persona::CLIENTE_RUC){
        this->tipo_persona_id = tipo_persona_id;
        this->persona_id = persona_id;
        ui->lineEdit_codigo->setText(codigo);
        ui->lineEdit_nombre->setText(nombre);
...
void VentaFactura::set_data(int tipo, QString id, QString persona_id, QString tipo_persona_id
...
void VentaFactura::set_time()
{
    ui->dateTimeEdit_sistema->setDateTime(QDateTime::currentDateTime());
}

void VentaFactura::on_pushButton_transformar_clicked()
{
    ...
    case 0:{
        ...
        if(id.compare("") == 0){
            ...
            w->set_data(venta_items::FACTURA, id, persona_id, tipo_persona_id
            , fecha_emision, ""
            , serie, numero
            , codigo, nombre
            , direccion, productos);
...
void VentaFactura::on_cliente_closing()
{
...
void VentaFactura::on_producto_closing()
{
...
void VentaFactura::on_pushButton_cliente_clicked()
{
...
void VentaFactura::on_pushButton_ing_prod_clicked()
{
...
void VentaFactura::on_pushButton_guardar_clicked()
{
    ...
    if(!(ui->lineEdit_serie->text().compare(serie) == 0 && ui->lineEdit_numero->text().compare(numero) == 0)){
        QString str_query = "(SELECT IF(EXISTS(SELECT 1 FROM factura";
        ...
        str_query += " JOIN anexo ON anexo.documento_id = factura.comprobante_documento_id";
        str_query += " ORDER BY factura.comprobante_documento_id DESC, anexo.numero DESC LIMIT 1)";
...
void VentaFactura::on_pushButton_eliminar_clicked()
{
    if(id.compare("") == 0) {
...
void VentaFactura::on_pushButton_imprimir_clicked()
{
    if(id.compare("") == 0){
        return;
    }
    ...
    if(ui->lineEdit_serie->text().length() == 4){
        ...
        str_query += " AND series.tipo_documento_id = "+QString().setNum(tipo_documento::FACTURA);
    ...
    w->set_tipo_documento(tipo_documento::FACTURA, ui->lineEdit_serie->text(), id_series
                          , labels, width, height, margen_fila, margen_columna);

    SYSTEM->change_center_w(this, w);
}

void VentaFactura::on_pushButton_amarres_clicked()
{

}

void VentaFactura::on_pushButton_salir_clicked()
{
...
void VentaFactura::showEvent(QShowEvent *se)
{
    if(focusWidget()){
...
void VentaFactura::hideEvent(QHideEvent *he)
{
    emit hiding();
    he->accept();
}
void VentaFactura::closeEvent(QCloseEvent *ce)
{
    emit closing();
    ce->accept();
}
bool VentaFactura::eventFilter(QObject *obj, QEvent *e)
{
    QWidget* w_temp;
    w_temp = this;
    if(obj == w_temp){
...
void VentaFactura::on_pushButton_up_clicked()
{
    QTableWidgetItem* item= ui->tableWidget->currentItem();
    if(item){
...
void VentaFactura::on_pushButton_down_clicked()
{
    QTableWidgetItem* item= ui->tableWidget->currentItem();
    if(item){
        int column = item->column();
...
void VentaFactura::on_pushButton_borrar_clicked()
{
    QTableWidgetItem* item= ui->tableWidget->currentItem();
    if(!item)return;
    int row = item->row();
...
void VentaFactura::on_lineEdit_serie_returnPressed()
{
...
void VentaFactura::on_tableWidget_itemChanged(QTableWidgetItem *item)
{
    disconnect(ui->tableWidget, SIGNAL(itemChanged(QTableWidgetItem*))
               , this, SLOT(on_tableWidget_itemChanged(QTableWidgetItem*)));
...
void VentaFactura::on_pushButton_anular_clicked()
{
    if(id.compare("") == 0){
    ...
    QString str_query = "UPDATE factura SET";
    str_query += " anulado = '"+QString().setNum(anulado)+"'";
    str_query += " WHERE comprobante_documento_id = "+id;
...
void VentaFactura::on_dateEdit_declaracion_dateChanged(const QDate &date)
{
...
void VentaFactura::on_dateEdit_emision_dateChanged(const QDate &date)
{
...
void VentaFactura::on_registroSinDoc_buscar_closing()
{
...
void VentaFactura::on_notaPedido_buscar_closing()
{
    VentaBuscar* w = (VentaBuscar*)sender();
...
void VentaFactura::on_cotizacion_buscar_closing()
{
    VentaBuscar* w = (VentaBuscar*)sender();
...
void VentaFactura::on_guiarr_buscar_closing()
{
    VentaBuscar* w = (VentaBuscar*)sender();
...
void VentaFactura::on_pushButton_buscar_registroSinDoc_clicked()
{
    if(persona_id.compare("") == 0){
...
void VentaFactura::on_pushButton_jalar_registroSinDoc_clicked()
{
...
void VentaFactura::on_pushButton_quitar_registroSinDoc_clicked()
{
...
void VentaFactura::on_pushButton_buscar_notaPedido_clicked()
{
    if(persona_id.compare("") == 0){
        QMessageBox::warning(this, "Advertencia", "No ha ingresado el cliente.", "Ok");
        return;
...
void VentaFactura::on_pushButton_jalar_notaPedido_clicked()
{
...
void VentaFactura::on_pushButton_quitar_notaPedido_clicked()
{
...
void VentaFactura::on_pushButton_buscar_cotizacion_clicked()
{
    if(persona_id.compare("") == 0){
        QMessageBox::warning(this, "Advertencia", "No ha ingresado el cliente.", "Ok");
        return;
...
void VentaFactura::on_pushButton_jalar_cotizacion_clicked()
{
...
void VentaFactura::on_pushButton_quitar_cotizacion_clicked()
{
...
void VentaFactura::on_pushButton_buscar_guiarr_clicked()
{
    if(persona_id.compare("") == 0){
        QMessageBox::warning(this, "Advertencia", "No ha ingresado el cliente.", "Ok");
        return;
...
void VentaFactura::on_pushButton_jalar_guiarr_clicked()
{
...
void VentaFactura::on_pushButton_quitar_guiarr_clicked()
{

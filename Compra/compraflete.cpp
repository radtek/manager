    ...
    ui(new Ui::CompraFlete)
{
    ...
    ui->tableWidget->setItemDelegateForColumn(INDEX_P_U_FLETE, dSpinBoxDelegate);
    ...
    ui->tableWidget->setItemDelegateForColumn(INDEX_P_T_FLETE, dSpinBoxDelegate);
    ...
    ui->doubleSpinBox_total->installEventFilter(this);
                         ...
                         , QString codigo, QString nombre)
{
    ...
    str_query += ", concat(producto.descripcion,' ',marca.marca), d_h_prod.precio, d_h_prod.precio_aux FROM documento";
...
{    
    ...
    w_buscar_transportista->set_tipo(persona_items::TRANSPORTISTA);
...
void CompraFlete::on_pushButton_calcular_clicked()
{
    ...
    for(int i = 0; i < ui->tableWidget->rowCount(); i++){
        ...
        ui->tableWidget->item(i, INDEX_P_U_FLETE)->setText(QString().setNum(p_unitario_flete, ' ', DECIMALS_PRECIO_UNITARIO));
        ui->tableWidget->item(i, INDEX_P_T_FLETE)->setText(QString().setNum(p_total_flete, ' ', DECIMALS_PRECIO_TOTAL));
    }
